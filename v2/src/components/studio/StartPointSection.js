...
export default function StartPointSection({ value, onChange, images = [], onUploadAccommodationPhoto, onUploadPoiPhoto, uploadLoading = false }) {
  ...
  const uploadPoiPhoto = async (file, index) => {
    ...
    if (!media?.id) return;
    update({ pois: point.pois.map((item, itemIndex) => itemIndex === index ? { ...item, photo_url: "", photoMediaId: media.id } : item) });
  };
      ...
      <section className="studio-section-block">
        ...
        {point.pois.map((item, index) => <article ref={newItem.type === "poi" && newItem.index === index ? newItemRef : null} className="studio-subitem-card" key={`start-poi-${index}`}>
          <div className="studio-form-grid studio-form-grid--compact">
            ...
            <div className="studio-form-grid__full">
              <label htmlFor={`start-poi-photo-${index}`}>Photo (URL ou fichier)</label>
              <div className="studio-resource-field">
                <input id={`start-poi-photo-${index}`} type="url" value={item.photo_url} onChange={e => update({ pois: point.pois.map((poi, itemIndex) => itemIndex === index ? { ...poi, photo_url: e.target.value, photoMediaId: null } : poi) })} />
