...
function DuplicatePoiControl({ poi, stageId, variantId, stages, variantsByStage, onDuplicate }) {
  ...
  const duplicate = async () => {
    ...
    setDuplicating(true);
    try {
      const duplicated = await onDuplicate?.(target, poi);
      ...
      <div className="studio-sublist__list">
        ...
        {stagePois.map(poi => (
          <article key={poi.id} className="studio-subitem-card">
            ...
            <DuplicatePoiControl poi={poi} stageId={stageId} variantId={variantId} stages={stages} variantsByStage={variantsByStage} onDuplicate={onDuplicate} />
          </article>
        ))}
      </div>
      {isEditingHere && (
