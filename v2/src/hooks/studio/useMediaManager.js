...
export function useMediaManager({ supabase, roadbookId, userId, onError, onSuccess, onMutation }) {
  ...
  const uploadMedia = useCallback(async (file, { stageId = null, variantId = null, metadata = {} } = {}) => {
    if (!file) return;
    setUploadError(null);
    ...
    try {
      ...
      const record = {
        ...
        uploaded_by: userId,
        metadata: { ...metadata, original_name: file.name, original_size: file.size, resized_width: width, resized_height: height, final_size: size, format: "jpeg", ...(variantId ? { variant_id: Number(variantId) } : {}) },
      };
