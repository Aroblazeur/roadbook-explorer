  ...
  return (
    <StudioShell>
      ...
      {success && <p className="page-success">{success}</p>}
          ...
          <GpxSection gpxError={gpxError} gpxOfficial={gpxOfficial} gpxCustom={gpxCustom} gpxUploading={gpxUploading} handleGpxDownload={(row) => downloadGpx(row)} handleGpxReplace={(row, scope, role, stageId, variantId) => { const input = document.createElement("input"); input.type = "file"; input.accept = ".gpx"; input.onchange = async () => { const file = input.files?.[0]; if (!file) return; await replaceGpx(file, row, { scope, role, stageId, variantId }); }; input.click(); }} handleGpxDelete={(row) => { if (!window.confirm("Supprimer ce GPX ?")) return; deleteGpx(row); }} handleGpxUpload={(scope, role, stageId, variantId) => { const input = document.createElement("input"); input.type = "file"; input.accept = ".gpx"; input.onchange = async () => { const file = input.files?.[0]; if (!file) return; await uploadGpxFile(file, { scope, role, stageId, variantId }); }; input.click(); }} />
