...
export default function RoadbookDetailPage() {
  ...
  const { id } = useParams();
  const { roadbook, setRoadbook, stages, setStages, poisByStage, setPoisByStage, variantsByStage, setVariantsByStage, loading, fetchError, setFetchError, loadAll, reloadStages, reloadPoisVariants, refreshRoadbookVersion } = useRoadbookData({ supabase, roadbookId: id, user, enabled: !authLoading && !!user && !!id });
