...
if (!dryRun && !serviceRoleKey) {
  console.error("Error: SUPABASE_SERVICE_ROLE_KEY not set in .env.local. Add it to run the import.");
  process.exit(1);
}
  ...
  if (Array.isArray(noteItems)) {
    for (const item of noteItems) {
      if (typeof item === "string") {
        if (item.trim()) result.push({ text: item.trim() });
      } else if (item && item.text) {
        result.push({ text: item.text, photo: item.photo || null, createdAt: item.createdAt || null, source: item.source || null });
      }
    }
  }
  if (notes && typeof notes === "string" && notes.trim()) {
    result.push({ text: notes.trim() });
  }
...
function formatCount(items) {
  ...
  for (const item of items) {
    const label = item.itemType || "unknown";
...
if (!fs.existsSync(jsonPath)) {
  ...
  console.error("Expected path structure: roadbooks/<slug>/roadbook.json");
  process.exit(1);
}
...
if (dryRun) {
  ...
  for (const s of stages) {
    ...
    for (const list of [s.pois, s.pointsOfInterest, s.interest]) {
      if (Array.isArray(list)) {
        for (const p of list) if (p && p.name) pois.add(p.name);
      }
    }
    totalPois += pois.size;
  ...
  // Unmapped fields at roadbook level
  ...
  // Unmapped fields at stage level
  ...
  for (const s of stages) {
    ...
    for (const sub of (s.substeps || [])) {
      collectUnmappedFields(sub, stageKnownKeys).forEach(k => stageUnmapped.add(k));
    }
  }
...
const isPublic = v1.metadata?.projectStatus === "public" || true;
...
if (existingRb) {
  ...
  const { data: newRb, error: insertErr } = await supabase
    ...
    .select("id")
    .single();
  if (insertErr) { report.errors.push(`Insert roadbook: ${insertErr.message}`); console.error(insertErr); process.exit(1); }
...
for (const s of stages) {
  ...
  if (s.accommodation?.alternatives) {
    for (const alt of s.accommodation.alternatives) {
      if (alt && (alt.url || alt.name)) {
        accommodationAlternatives.push({
          url: alt.url || "",
          name: alt.name || "",
          photo: alt.photo || "",
        });
  ...
  if (Array.isArray(s.warning) && s.warning.length && !notesArray.some(n => n.type === "warning")) {
    // warnings already handled in notes, but also keep in metadata
    stageMetadata.warnings = s.warning;
  }
  ...
  if (existingStage) {
    if (!upsert) {
      ...
      else { report.updated.stages++; console.log(`  Stage ${stageNumber} "${s.name}": updated`); }
      stageId = existingStage.id;
    }
  } else {
  ...
  for (const p of rawPois) {
    if (p && p.name && !stagePoiNames.has(p.name)) {
      stagePoiNames.add(p.name);
      uniquePois.push(p);
    }
  }
  ...
  for (const p of uniquePois) {
    ...
    const poiPayload = {
      stage_id: stageId,
      name: p.name,
      lat: enriched?.coordinates?.lat || null,
      lng: enriched?.coordinates?.lng || null,
      poi_type: null,
      description: enriched?.description || null,
      photo_url: p.image || enriched?.image || null,
      link_url: p.url || null,
  ...
  if (Array.isArray(s.substeps)) {
    ...
    for (const sub of s.substeps) {
      ...
      if (sub.accommodation) variantMetadata.accommodation = {
        name: sub.accommodation.name || "",
        url: sub.accommodation.url || sub.accommodation.website || "",
        photo: sub.accommodation.photo || "",
        alternatives: sub.accommodation.alternatives || [],
      };
      ...
      // Store variant-specific fields in metadata since some columns may not exist yet on stage_variants
      ...
      const variantPayload = {
        stage_id: stageId,
        label: sub.name || sub.title || "Variante",
        distance_km: sub.distance || null,
        gpx_url: sub.gpx || null,
...
if (report.unmappedFields.size) {
  console.log(`\nUnmapped fields (stored in metadata JSONB):`);
  for (const f of report.unmappedFields) console.log(`  - ${f}`);
}
if (report.errors.length) {
  console.log(`\nErrors (${report.errors.length}):`);
  for (const e of report.errors) console.log(`  - ${e}`);
}
console.log("\nNote: GPX files are referenced by relative paths (e.g. 'gpx/etape01.gpx'). Storage upload not yet implemented.");
console.log("Note: Stage photos, POI photos, and accommodation photos are stored as URLs in existing columns. No media table records created.");
console.log("=== Done ===\n");
